...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (!origin || allowed.includes('*') || allowed.some(o => origin.startsWith(o) || origin.includes('.devtunnels.ms'))) {
      callback(null, true);
    } else {
...
app.use(express.json({ limit: '50mb' })); // Allow large base64 images/videos
